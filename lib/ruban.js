...
export function tracerRuban(pts, lever, coucher, aujourdhui) {
  const L = 1000, H = 190;
  const haut = 18, bas = 89, sol = 136, yTexte = 174;
  const marge = 40;
  ...
  const pluie = pts.map((p, i) => p.pluie > 10
    ? `<rect x="${X(i)-large/2}" y="${sol - p.pluie/100*34}" width="${large}" height="${p.pluie/100*34}"
  ...
  const et = i => `<text class="degre" x="${X(i)}" y="${Y(T[i]) + (i === iMax ? -18 : 31)}"
